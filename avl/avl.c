...
/* Creates and returns a _new table
...
avl_probe (struct avl_table *tree, void *item)
{
  ...
  struct avl_node *w;     /* _new root of rebalanced subtree. */
  ...
  n = q->avl_link[dir] = (struct avl_node *)
...
/* Replaces the current item in |trav| by |_new| and returns the item replaced.
   |trav| must not have the null item selected.
   The _new item must not upset the ordering of the tree. */
void *
avl_t_replace (struct avl_traverser *trav, void *_new)
{
  void * old;

  assert (trav != NULL && trav->avl_node != NULL && _new != NULL);
  old = trav->avl_node->avl_data;
  trav->avl_node->avl_data = _new;
  return old;
}

/* Destroys |_new| with |avl_destroy (_new, destroy)|,
   first setting right links of nodes in |stack| within |_new|
   to null pointers to avoid touching uninitialized data. */
static void
copy_error_recovery (struct avl_node **stack, int height,
                     struct avl_table *_new, avl_item_func *destroy)
{
  assert (stack != NULL && height >= 0 && _new != NULL);

  for (; height > 2; height -= 2)
    stack[height - 1]->avl_link[1] = NULL;
  avl_destroy (_new, destroy);
}
...
/* Copies |org| to a newly created tree, which is returned.
   ...
   On failure, destroys the partially created _new tree,
   applying |destroy|, if non-null, to each item in the _new tree so far,
   and returns |NULL|.
   If |allocator != NULL|, it is used for allocation in the _new tree.
          ...
          avl_item_func *destroy, struct libavl_allocator *allocator)
{
  ...
  struct avl_table *_new;
  const struct avl_node *x;
  struct avl_node *y;

  assert (org != NULL);
  _new = avl_create (org->avl_compare, org->avl_param,
                    allocator != NULL ? allocator : org->avl_alloc);
  if (_new == NULL)
    return NULL;
  _new->avl_count = org->avl_count;
  if (_new->avl_count == 0)
    return _new;

  x = (const struct avl_node *) &org->avl_root;
  y = (struct avl_node *) &_new->avl_root;
  for (;;)
    {
      while (x->avl_link[0] != NULL)
        {
          ...
          y->avl_link[0] =
            _new->avl_alloc->libavl_malloc (_new->avl_alloc,
                                           sizeof *y->avl_link[0]);
          if (y->avl_link[0] ==  NULL)
            {
              if (y != (struct avl_node *) &_new->avl_root)
                {
                  y->avl_data = NULL;
                  y->avl_link[1] = NULL;
                }

              copy_error_recovery (stack, height, _new, destroy);
              return NULL;
            }
